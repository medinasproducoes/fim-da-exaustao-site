// _worker.js — O Fim da Exaustão Digital

async function criarPagamento(request) {
  const headers = { 'Content-Type': 'application/json' };
  try {
    const SITE_URL = new URL(request.url).origin;
    const orderNsu = `exaustao-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const payload = {
      handle: 'medina_pay',
      items: [{ quantity: 1, price: 3790, description: 'O Fim da Exaustão Digital - Acesso Vitalício' }],
      order_nsu: orderNsu,
...
async function webhookInfinitePay(request) {
  try {
    ...
    console.log('Venda confirmada (Exaustão Digital):', data.order_nsu, data.paid_amount);
